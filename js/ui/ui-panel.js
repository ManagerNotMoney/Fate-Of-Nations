(function() {
    ...
    window.UIPanel = {
        ...
        _renderResidentsTab: function(bc, building) {
            const resData = window.PopulationEngine.getBuildingResidents(HM, building.col, building.row);
            if (!resData) return '';
        ...
        _renderDistrictTab: function(building) {
            const stats = window.PopulationEngine.getDistrictStats(HM, building.col, building.row);
            ...
            if (ideo && ideo.total > 0) {
                const rows = Object.entries(ideo.IDEOLOGY_META).map(([key, meta]) => {
                    const n = ideo[key] || 0;
                    if (n <= 0) return '';
        ...
        _renderMarketTab: function(building) {
            ...
            const nearbyPop = window.ConstructionEngine.getPopulationInRadius(HM, building.col, building.row, radius);
            const hasCompetitor = window.ConstructionEngine.hasNearbyMarket(HM, building.col, building.row);
        ...
        _bindCancelDemolishButtons: function(col, row) {
            ...
            if (cancelBtn) {
                cancelBtn.addEventListener('click', () => {
                    const result = window.ConstructionEngine.cancelBuild(HM, col, row);
                    if (result.ok) {
            ...
            if (demolishBtn) {
                demolishBtn.addEventListener('click', () => {
                    ...
                    const result = window.ConstructionEngine.demolishBuilding(HM, col, row);
                    if (result.ok) {
            ...
            if (upgradeBtn) {
                upgradeBtn.addEventListener('click', () => {
                    const result = window.ConstructionEngine.upgradeBuilding(HM, col, row);
                    if (result.ok) {
                        const b = HM.buildings[col + ',' + row];
                        const bc = C.BUILDINGS[b.type];
        ...
        _bindWorkerButtons: function(col, row) {
            ...
            if (addBtn) {
                addBtn.addEventListener('click', () => {
                    const result = window.PopulationEngine.assignWorker(HM, col, row);
                    if (result.ok) {
            ...
            if (removeBtn) {
                removeBtn.addEventListener('click', () => {
                    const result = window.PopulationEngine.removeWorker(HM, col, row);
                    if (result.ok) {
        ...
        _renderBuildActions: function(col, row, tile, building, inQueue) {
            ...
            const shouldHide = function(type, tileType) {
                return !window.ConstructionEngine.isTerrainCompatible(type, tileType);
            };

            const btns = Object.entries(C.BUILDINGS).map(([type, bc]) => {
                if (shouldHide(type, tile.type)) return '';

                const check = window.ConstructionEngine.canBuild(HM, col, row, type);

                // Dynamic cost
                const dynCost = window.ConstructionEngine.getDynamicCost(HM, type);
                let costHtml = '';
                if (type === 'local_admin') {
        ...
        build: function(col, row, type) {
            const result = window.ConstructionEngine.queueBuild(HM, col, row, type);
            if (result.ok) {
        ...
        quickAssignWorkers: function(col, row) {
            ...
            if ((building.assignedWorkers || 0) >= maxWorkers) {
                ...
                while ((building.assignedWorkers || 0) > 0) {
                    const r = window.PopulationEngine.removeWorker(HM, col, row);
                    if (!r.ok) break;
                    removed++;
                }
                if (window.UI) {
                ...
                while ((building.assignedWorkers || 0) < maxWorkers) {
                    const r = window.PopulationEngine.assignWorker(HM, col, row);
                    if (!r.ok) break;
                    added++;
                }
                if (added > 0) {
            ...
            if (HM.buildQueue.length > 0) {
                ...
                this.queueList.querySelectorAll('.queue-cancel-btn').forEach(btn => {
                    btn.addEventListener('click', (e) => {
                        ...
                        const result = window.ConstructionEngine.cancelBuild(HM, c, r);
                        if (result.ok) {
                            if (window.UI) window.UI.showNotification('↩️ Строительство отменено, монеты возвращены');
                            if (window.UI) window.UI.updateResourceBar();
                            this.updateQueue();
