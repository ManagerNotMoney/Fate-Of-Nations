(function() {
    ...
    window.GameConfig = {
        ...
        TILES: {
            ...
            river:   { color: '#1e4d7a', name: 'Река',              canBuild: false },
        ...
        MAP_SIZES: {
            ...
            huge: { cols: 80, rows: 62 }
        },

        MAP_TYPES: {
            auto:       { label: 'Случайный',   icon: '🎲', desc: 'Каждый раз разный' },
            continent:  { label: 'Континент',   icon: '🌍', desc: 'Суша в центре, море по краям' },
            archipelago:{ label: 'Архипелаг',   icon: '🏝️', desc: 'Острова в океане' },
            mixed:      { label: 'Смешанный',   icon: '🗺️', desc: 'Без явных границ' },
            rivers:     { label: 'Речной край', icon: '🌊', desc: 'Много рек и плодородных земель' }
        },
    ...
    window.hexKey = function(col, row) { return col + ',' + row; };
})();
