    ...
    window.EventsEngine = {
            ...
            for (const effect of this._activeEffects) {
                if (effect.type === 'drought') {
                    for (const b of Object.values(hexMap.buildings)) {
                        if (b.type === 'farm')    deltas.wheat  -= Math.min(effect.reduction, deltas.wheat);
                        if (b.type === 'orchard') deltas.apples -= Math.min(effect.reduction, deltas.apples);
                    }
                }
                if (effect.type === 'locust') {
                    for (const b of Object.values(hexMap.buildings)) {
                        if (b.type === 'farm') deltas.wheat -= Math.min(3, deltas.wheat);
                    }
                }
            }
        },
        ...
        REGISTRY: [
            ...
            // ── 3. НАБЕГ ─────────────────────────────────
            {
                ...
                apply: function(hm) {
                    if (hm.resources.defense < Math.floor(hm.resources.population)) {
                        ...
                        const needed = Math.floor(hm.resources.population);
                        hm.resources.defense = Math.max(0, hm.resources.defense - needed);
            ...
            // ── 4. ХОРОШИЙ УРОЖАЙ ────────────────────────
            {
                ...
                condition: function(hm) {
                    return Object.values(hm.buildings).some(b => b.type === 'farm' && window.PopulationEngine.isBuildingActive(hm, b.col, b.row));
                },
                apply: function(hm) {
                    const farms = Object.values(hm.buildings).filter(b => b.type === 'farm' && window.PopulationEngine.isBuildingActive(hm, b.col, b.row));
            ...
            // ── 5. МОРСКАЯ УДАЧА ─────────────────────────
            {
                ...
                condition: function(hm) {
                    return Object.values(hm.buildings).some(b => b.type === 'port' && window.PopulationEngine.isBuildingActive(hm, b.col, b.row));
                },
                apply: function(hm) {
                    const ports = Object.values(hm.buildings).filter(b => b.type === 'port' && window.PopulationEngine.isBuildingActive(hm, b.col, b.row));
            ...
            // ── 6. АЛМАЗЫ ────────────────────────────────
            {
                ...
                condition: function(hm) {
                    return Object.values(hm.buildings).some(b => b.type === 'mine' && window.PopulationEngine.isBuildingActive(hm, b.col, b.row));
                },
                apply: function(hm) {
            ...
            // ── 7. БОЛЬШЕ ЯБЛОК ──────────────────────────
            {
                ...
                condition: function(hm) {
                    return Object.values(hm.buildings).some(b => b.type === 'orchard' && window.PopulationEngine.isBuildingActive(hm, b.col, b.row));
                },
                apply: function(hm) {
                    const orchards = Object.values(hm.buildings).filter(b => b.type === 'orchard' && window.PopulationEngine.isBuildingActive(hm, b.col, b.row));
            ...
            // ── 8. ВИШНЁВЫЙ БУМ ──────────────────────────
            {
                ...
                condition: function(hm) {
                    return Object.values(hm.buildings).some(b => b.type === 'cherry_orchard' && window.PopulationEngine.isBuildingActive(hm, b.col, b.row));
                },
                apply: function(hm) {
                    const cherry_orchards = Object.values(hm.buildings).filter(b => b.type === 'cherry_orchard' && window.PopulationEngine.isBuildingActive(hm, b.col, b.row));
                    const bonusPer = 3 + Math.floor(Math.random() * 3);
                        ...
                        choices: [
                            {
                                ...
                                apply: function(hm) {
                                    ...
                                    // population не трогаем — она не была уменьшена
            ...
            // ── 5. САРАНЧА ───────────────────────────────
            {
                ...
                apply: function(hm) {
                    if (window.EventsEngine._activeEffects.some(e => e.type === 'locust')) {
                        return { message: 'Саранча уже хозяйничает на полях.'};
                    }
                    ...
                    return {
                        ...
                        targetCol: affectedFarms[0].col,
                        targetRow: affectedFarms[0].row,
                        choices: [
                            {
            ...
            // ── 7. ДОЛОЙ БУРЖУАЗИЮ! ───────────────────────
            {
                ...
                condition: function(hm) {
                    // Считаем коммунистов только в mine / factory / cherry_orchard
                    const ideo = window.IdeologyEngine.getGlobalIdeologies(hm);
                    const communistCount = ideo.communist;
                ...
                apply: function(hm) {
                    // Получаем коммунистские здания, но оставляем только нужные типы
                    const communistBuildings = window.IdeologyEngine.getBuildingsByIdeology(hm, 'communist', true);
